...
runQuery("INSERT INTO `user_order` (`id`, `user_id`, `menu_id`, `status`, `unique_number`, `created_date`, `address`, `payment_method`, `delivery_method`, `payment_receipt`, `payer_name`, `size`) VALUES (NULL, '".$_SESSION['account_session']."', '".json_encode($cartCollection)."', '1', '$FourDigitRandomNumber', current_timestamp(), '$payer_address', '1', '$payer_method', '$fileName', '$payer_name', '".json_encode($cartSizeStore)."')");
echo "<script>alert('Purchase done, Thank You!'); window.location.href='user-cart.php';</script>";
?>