<?php
    ...
    $menuorder = fetchRows("SELECT * FROM `user_order` WHERE `user_id` =".$userid);
?>
    ...
    <div class="surface-0 border-round-2xl h-full shadow-3 content-concreate-bg p-3">
        ...
        <div class="h-full w-full flex align-items-center justify-content-center flex-column gap-3">
            <div class="max-h-30rem overflow-auto">
                <table border="1" cellspacing="0" cellpadding="20" class="surface-0">
                    <tr>
                        <th>No.</th>
                        <th>Watch</th>
                        <th>Buyer Info</th>
                    ...
                    <?php
                        if(!empty($menuorder)){
                            ...
                            echo '<tr><td align="center" colspan="4">No Item Yet</td></tr>';
                        }
                    ?>
                </table>    
            </div>

            <a href="user-shop.php" class="no-underline text-2xl bg-bluegray-500 cursor-pointer py-2 px-6 h-4rem border-3 border-bluegray-600 hover:border-bluegray-800 border-circle border-3 text-0 flex align-items-center justify-content-center">Close</a>
        </div>
        <!-- END Content-->
